...
const AddWaterScreen = props => {
  ...
  React.useEffect(() => {
    setNeedValue(hasCredit ? Math.min(15.5, parseFloat((Math.round((parseFloat(specification[0].weightSize) * 41.6150228) * 2 / 240) / 2).toFixed(1))) : 8)
    getWater()
  }, [selectedDate])
  ...
  const getSuccess = (response) => {
    ...
    SW.syncWaterByLocal(waterDB, response.data.data, selectedDate, waterBulkSync, getFromDB)
  }
  ...
  const getFromDB = () => {
    ...
    waterDB.find({
      ...
      records.docs.map(item => { water = item.value })
      setValue(water)
  ...
  const getMealsSuccess = (response, selectedDate) => {
    ...
    SP.syncMealsByLocal(mealDB, response.data.data, selectedDate, mealBulkSync, getMealFromDB)
  }
  ...
  const saveDB = (data) => {
    ...
    waterDB.put({ ...data }).then(() => getFromDB()).catch(error => console.log(error))
    setLoading(false)
  ...
  let remainWater = hasCredit ? needValue - consumptionValue - otherWater : needValue - consumptionValue
      ...
      >
        ...
        <View style={styles.container}>
          <View style={{ flexDirection: "row", alignItems: "center" }}>
            ...
            <Text style={{ fontFamily: lang.font, fontSize: moderateScale(20), color: defaultTheme.blue }}> {lang.glass}</Text>
          </View>

          <Text style={[styles.text, { fontFamily: lang.titleFont, marginBottom: moderateScale(16) }]}>
            {lang.baghimande}
          </Text>
          ...
          <RowCenter style={{ marginTop: moderateScale(16), flexDirection: I18nManager.isRTL ? "row-reverse" : "row" }}>
          ...
          <Text style={[styles.text1, { fontFamily: lang.titleFont, marginTop: moderateScale(8) }]}>
            {lang.glass}
          </Text>
          ...
          <View style={{ flexDirection: "row", justifyContent: 'space-around', width: dimensions.WINDOW_WIDTH, marginVertical: moderateScale(50),marginTop:moderateScale(30) }}>
            ...
            <View style={styles.circleHeader}>
              <View style={{ flexDirection: "row", alignItems: "center" }}>
                {
                  hasCredit ? <Text style={[styles.circleHeaderTxt, { fontFamily: lang.titleFont }]}>{(hasCredit ? otherWater.toFixed(1) : "")}</Text>
                    :
                    <Image
                      source={require("../../../res/img/lock.png")}
                      style={{ width: moderateScale(23), height: moderateScale(23), tintColor: defaultTheme.primaryColor, resizeMode: "contain" }}
                    />

                }

                <Text style={{ color: defaultTheme.blue, fontSize: moderateScale(17), fontFamily: lang.titleFont }}> {lang.glass}</Text>

              </View >
              <Text style={{ fontFamily: lang.font, fontSize: moderateScale(15), marginTop: Platform.OS == 'ios' ? moderateScale(9) : 0 }}>{lang.drinkWaterOtherFood}</Text>
            </View>
            <View style={styles.circleHeader}>
              <View style={{ flexDirection: "row", alignItems: "center" }}>
                <Text style={[styles.circleHeaderTxt, { fontFamily: lang.titleFont }]}>{consumptionValue.toFixed(1)}</Text>
                <Text style={{ color: defaultTheme.blue, fontSize: moderateScale(17), fontFamily: lang.titleFont }}> {lang.glass}</Text>
              </View>
              <Text style={{ fontFamily: lang.font, fontSize: moderateScale(15), marginTop: Platform.OS == 'ios' ? moderateScale(9) : 0 }}>{lang.drinkingWater}</Text>
            </View>
            <View style={styles.circleHeader}>
              <View style={{ flexDirection: "row", alignItems: "center" }}>
                <Text style={[styles.circleHeaderTxt, { fontFamily: lang.titleFont }]}>{needValue}</Text>
                <Text style={{ color: defaultTheme.blue, fontSize: moderateScale(17), fontFamily: lang.titleFont }}> {lang.glass}</Text>
              </View>
              <Text style={{ fontFamily: lang.font, fontSize: moderateScale(15), marginTop: Platform.OS == 'ios' ? moderateScale(9) : 0 }}>{lang.dailyTarget}</Text>
            </View>
          </View>
        </View>
