...
import {MealDetailsCard, RowSpaceBetween, Toolbar} from '../../components';
...
const NutritionDetailsScreen = props => {
    ...
    mealDB
      ...
      .then(records => {
        ...
        new Array(34).fill(0).map((item, index) => {
          ...
          meals.map((meal, i) => {
            const fn =
              typeof meal.foodNutrientValue === 'string'
                ? meal.foodNutrientValue.split(',').map(i => parseFloat(i))
                : meal.foodNutrientValue.map(i => parseFloat(i));
            index === 23
              ? (value += parseInt(fn[index]))
              : (value += parseFloat(fn[index]));
          });
  ...
  unblurComponents = nutritions.map(
    (item, index) =>
      item.id != 1 &&
      item.id != 2 &&
      item.id != 10 &&
      ...
      item.id != 32 && (
        <RowSpaceBetween
          style={[styles.rowContainer, {paddingHorizontal: 0}]}
          key={index.toString()}>
          <Text
            style={[
              styles.text,
          ...
          <RowSpaceBetween
            style={[styles.row, {width: dimensions.WINDOW_WIDTH * 0.65}]}>
            ...
            {!hasCredit ? (
              ...
              <Text
                ...
                allowFontScaling={false}>
                {mealsNutirtions[parseInt(item.id) - 1] &&
                  ...
                  <Text
                    style={[
                      styles.text,
                      {
                        fontFamily: lang.font,
                        color:
                          parseFloat(nutritionsValue[parseInt(item.id) - 1]) -
                            parseFloat(
                              mealsNutirtions[parseInt(item.id) - 1],
                            ) >=
                          0
                    ...
                    allowFontScaling={false}>
                    {parseFloat(nutritionsValue[parseInt(item.id) - 1]) -
                      parseFloat(mealsNutirtions[parseInt(item.id) - 1]) >
                    0
                      ? (
                          parseFloat(nutritionsValue[parseInt(item.id) - 1]) -
                          parseFloat(mealsNutirtions[parseInt(item.id) - 1])
        ...
        <ScrollView
          ...
          <RowSpaceBetween
            style={[
              ...
              {paddingHorizontal: 0, marginVertical: moderateScale(5)},
            ]}>
            <Text
              style={[
                styles.title,
                {
                  fontFamily:
                    lang.langName === 'persian' ? lang.font : lang.titleFont,
                },
              ]}
              allowFontScaling={false}>
              {lang.bigNutrition}
