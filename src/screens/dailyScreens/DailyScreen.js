...
import { Information, Calendar, MainToolbar, DailyActivityContainer, DailyFoodContainer, RowSpaceBetween, ConfirmButton, DatePicker, TwoOptionModal, DietCard } from '../../components';
...
const DailyScreen = props => {
  ...
  React.useEffect(() => {
    const starRatingTimer = moment(starRating.starRatingTimer, "YYYY-MM-DDTHH:mm:ss")
    starRatingTimer.diff(today, "seconds") > 0 ? setShowRating(false) : setShowRating(true)
  ...
  const getPedofromDB = (date) => {
    ...
    pedoDB.allDocs({
      ...
      setPedos([...records.rows.map(item => item.doc).filter(item => item.insertDate.includes(date))])
    })
  }

  const nextDayPressed = () => {
    setCalDisabledBtn(true)
    let nDay = selectedDate === "2021-03-21" ? "2021-03-22" : selectedDate === "2021-03-22" ? "2021-03-23" : moment(selectedDate, 'YYYY-MM-DD').add(1, "day").format('YYYY-MM-DD')
    setDate(nDay)
  ...
  const prevDayPressed = () => {
    setCalDisabledBtn(true)
    let pDay = selectedDate === "2021-03-23" ? "2021-03-22" : selectedDate === "2021-03-22" ? "2021-03-21" : moment(selectedDate, 'YYYY-MM-DD').subtract(1, "days").format('YYYY-MM-DD')
    setDate(pDay)
  ...
  const addBreakfast = () => {
    props.navigation.navigate("FoodFindScreen", { type: "", name: lang.breakfast, mealId: 0 })
  }

  const addLunch = () => {
    props.navigation.navigate("FoodFindScreen", { type: "", name: lang.lunch, mealId: 1 })
  }

  const addSnack = () => {
    props.navigation.navigate("FoodFindScreen", { type: "", name: lang.snack, mealId: 2 })
  }

  const addDinner = () => {
    props.navigation.navigate("FoodFindScreen", { type: "", name: lang.dinner, mealId: 3 })
  }
  ...
  const addCalorie = () => {
    if (hasCredit) {
      props.navigation.navigate("AddCaloryScreen", { selectedDate: selectedDate })
    }
    else {
      goToPackages()
    }
  ...
  const editMeal = (item) => {
    if (item.foodId === null && item.personalFoodId === null) {
      ...
      props.navigation.navigate("FoodDetailScreen", { meal: item, food: { ...item, foodMeal: item.foodMeal, name: item.foodName, measureUnitName: undefined } })
    }
  }
  ...
  const onActivityEdit = async (item) => {
    ...
    if (item.stepsCount) {
      ...
      props.navigation.navigate("ActivityDetailsScreen", { activity: item, date: date })
    }
  }
  ...
  const deleteConfirmed = () => {
    ...
    !isNaN(parseInt(deleteAction.foodMeal)) ? removeMeal(deleteAction) :
  ...
  const deleteActivity = (item) => {
    if (item.stepsCount) {
      if (app.networkConnectivity) {
        ...
        const RC = new RestController()
        RC.checkPrerequisites("delete", url, params, header, (res) => deleteActivitySuccess(res, item), deleteActivityFailure, auth, onRefreshTokenSuccess, onRefreshTokenFailure)
      }
      else {
        offlineDB.post({
          method: "delete",
  ...
  const syncMeal = (date) => {
    ...
    const RC = new RestController()
    RC.checkPrerequisites("get", url, params, header, (res) => getMealsSuccess(res, date), getMealsFailure, auth, onRefreshTokenSuccess, onRefreshTokenFailure)
  }
  ...
  const getMealsSuccess = (response, selectedDate) => {
    ...
    SP.syncMealsByLocal(mealDB, response.data.data, selectedDate, mealBulkSync, () => getMealFromDB(selectedDate, true))
  }
  ...
  const getPedoFromServer = (date) => {
    ...
    const RC = new RestController()
    RC.checkPrerequisites("get", url, params, header, (res) => getPedoSuccess(res, date), getPedoFailure, auth, onRefreshTokenSuccess, onRefreshTokenFailure)
  }

  const getPedoSuccess = (response, selectedDate) => {
    const SP = new SyncPedoDB()
    SP.syncStepsByLocal(pedoDB, response.data.data, selectedDate, stepBulkSync, getPedofromDB)
  }
  ...
  const syncActivities = (date) => {
    ...
    const RC = new RestController()
    RC.checkPrerequisites("get", url, params, header, (res) => getActivitySuccess(res, date), getActivityFailure, auth, onRefreshTokenSuccess, onRefreshTokenFailure)
  }

  const getActivitySuccess = (response, selectedDate) => {
    const SP = new SyncActivityDB()
    SP.syncActivitiesByLocal(activityDB, response.data.data, selectedDate, activityBulkSync, getActivityFromDB)
  }
    ...
    <>
      ...
      >
        ...
        <DailyActivityContainer
          ...
          onDelete={item => { setDeleteAction(item); showDeleteDialog(true) }}
...
const styles = StyleSheet.create({
  ...
  button1: {
    width: dimensions.WINDOW_WIDTH * .38,
