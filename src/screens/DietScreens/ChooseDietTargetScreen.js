...
function ChooseDietTargetScreen(props) {
  ...
  const [selectedKG, setSelectedKG] = useState(
    `${specification[0].weightSize.toString().split('.')[0]}`,
  );
  const [selectedGR, setSelectedGR] = useState(
    `${
      specification[0].weightSize.toString().split('.')[1] == undefined
        ? 0
        : specification[0].weightSize.toString().split('.')[1]
    }`,
  );
  const [selectedTergetKG, setSelectedTergetKG] = useState(
    `${profile.targetWeight.toString().split('.')[0]}`,
  );
  const [selectedTergetGR, setSelectedTergetGR] = useState(
    `${
      profile.targetWeight.toString().split('.')[1] == undefined
        ? 0
        : profile.targetWeight.toString().split('.')[1]
    }`,
  );
  ...
  const bodyTargetData = [
    {
      id: 0,
      name: lang.weightLoss,
      title: 'اگر میخواین لاغر بشین این گزینه رو انتخاب کنین',
    },
    {
      id: 1,
      name: lang.weightGain,
      title: 'اگر میخوایین چاق بشین این گزینه رو انتخاب کنین',
    },
    {
      id: 2,
      name: lang.weightStability,
      title: 'اگر میخواین وزن فعلی رو حفظ کنین این گزینه رو انتخاب کنین',
    },
  ];
  ...
  return (
    <SafeAreaView>
      <OFitToolBar BackPressed={() => props.navigation.goBack()} />
      ...
      <ScrollView style={{height: '87%'}}>
            ...
            {bodyTargetData.map(item => {
              return (
                <TouchableOpacity
                  ...
                  style={[
                    styles.subFirstComponent,
                    {
                      paddingBottom: moderateScale(10),
                      borderBottomWidth: item.id !== 2 ? 1 : 0,
        ...
        <View style={[styles.firstCard, {marginBottom: moderateScale(70)}]}>
          <View
            ...
            <Text
              style={[
                styles.secondComponentText,
                {fontFamily: lang.titleFont},
              ]}>
              {lang.targeDietWeight}
            </Text>
          </View>

          <TouchableOpacity
            onPress={openWeightModal}
            style={styles.secontCardComp}>
            <Text
              style={[styles.secondComponentsubText, {fontFamily: lang.font}]}>
              {lang.currentWeight}
            </Text>
            <View style={{flexDirection: 'row', alignItems: 'center'}}>
              <View
                ...
                <Text style={[styles.measureText, {fontFamily: lang.font}]}>
                  {lang.kgMeasureName}
                </Text>
                <Text style={[styles.measureText, {fontFamily: lang.font}]}>
                  {selectedKG}.{selectedGR}
                </Text>
              </View>
          ...
          <TouchableOpacity
            ...
            <View style={{flexDirection: 'row', alignItems: 'center'}}>
              <View
                ...
                <Text style={[styles.measureText, {fontFamily: lang.font}]}>
                  {selectedTergetKG}.{selectedTergetGR}
                </Text>
              </View>
              <TouchableOpacity
                onPress={openTargetWeightModal}
          ...
          <View style={{flexDirection: 'row', padding: moderateScale(5)}}>
            <Info width={moderateScale(20)} />
            <Text
              style={{
                textAlign: 'left',
                lineHeight: moderateScale(23),
                fontFamily: lang.font,
                width: dimensions.WINDOW_WIDTH * 0.77,
                marginHorizontal: moderateScale(5),
              }}>
              اگر میخواین وزن فعلی و وزن هدف رو تغییر بدین روی آیکون بعلاوه کنار
              هر کدوم بزنین و وزن رو ثبت کنین در غیر اینصورت ادامه رو بزنین
            </Text>
          </View>
        </View>
      </ScrollView>
      ...
      <Modal
        ...
        <Animated.View
          style={[
            {transform: [{translateY: translateYF}], zIndex: 50},
          ...
          <View
            ...
            <Text
              ...
              برای تنظیم وزن اعداد رو به پایین یا بالا بکشید
            </Text>

            <Info width={moderateScale(17)} />
          </View>
          <View style={{flexDirection: 'row'}}>
            <View style={{alignItems: 'center'}}>
              <Text
                style={{fontFamily: lang.font, fontSize: moderateScale(15)}}>
                {lang.gr}
              </Text>
              <Picker
                style={styles.picker}
      ...
      <Modal
        ...
        <Animated.View
          style={[
            {transform: [{translateY: translateYS}], zIndex: 50},
            styles.AnimatedModal,
          ]}>
          <Text
            style={{
