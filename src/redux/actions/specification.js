...
export const updateSpecification = (data, auth, app, user, callback = () => true, callbackFailure = () => false) => {
    return (dispatch) => {
        ...
        if (app.networkConnectivity) {
            ...
            RC.checkPrerequisites("put", url, params, header, (res) => onUpdateSpecificationSuccess(res, dispatch, callback), (err) => {
                console.error(err);
                callbackFailure(err)
            }, auth, onRefreshTokenSuccess, onRefreshTokenFailure)
        }
        else {
            offlineDB.post({
                method: "put",
